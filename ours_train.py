...
img_size = (256, 512)
n_classes = 3
weights_in_loss = [1., 5.81867525, 0.78376658]#change the tensor based on res
...
methodology1 = 'ours_hardthreshold_psp_pooling_resnet18_aug'
checkpoint_path1 = 'checkpoints/amodal4/' + methodology1 +'.pth.tar'

# Define dataloaders
if DATASET == 'Cityscapes':
    val_gt_list = sorted(glob.glob(os.path.join(dataset_dir, 'gt_manual', 'val') + '/*/*gt_manual.png'))[:100]
train_set = CSDataset('train2.csv', transform=transforms.Compose([Rescale(img_size), CSToTensor()]))
val_set = CSDataset('val2.csv', transform=transforms.Compose([Rescale(img_size), CSToTensor()]))
...
G = PSPNetShareEarlyLayer(backend='resnet18shareearlylayer', psp_size=512, pretrained=False, n_classes=3)
...
#print(os.path.isfile(checkpoint_path1))
if restore:
    if os.path.isfile(checkpoint_path1):
        state = torch.load(checkpoint_path1)
        epoch = state['epoch']
